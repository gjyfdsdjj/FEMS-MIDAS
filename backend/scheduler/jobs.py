...
#   5. alerts 테이블 기록
#
# [Job D] 5분 주기 RCF 기반 이상 감지  (cron: */5 * * * *)
#   1. 공장 목록 조회
#   2. sensor_logs_archive 최근 N개 데이터 조회
#           (TODO: 운영 환경에서는 sensor_logs 조회로 변경 예정)
#   3. RCF(Random Cut Forest) 기반 anomaly score 계산
#   4. score >= 임계값 이상인 데이터 탐지
#   5. 최대 score가 알림 임계값을 초과하면 알림 생성
#   6. 최근 Rule Based 알림이 존재하면 중복 알림 방지
#   7. alerts 테이블 기록 및 Telegram 발송
#
# [현재 구현 매핑]
...
#  - Job B/C/D:
#   run_job_b_update_environment_weights(),
#   run_job_c_monitor_alerts(),
#   run_job_d_rcf_analysis()
#
# - Job B: 현재 스텁
# - Job C: Rule Based 이상 감지
# - Job D: RCF 기반 패턴 이상 감지
# -----------------------------------------------------------------------------
...
try:
    from backend.services import rcf_anomaly_service
except Exception:
    try:
        from services import rcf_anomaly_service  # type: ignore
    except Exception:
        rcf_anomaly_service = None  # type: ignore
...
def run_job_d_rcf_analysis() -> dict[str, Any]:
    """Job D: 5분 주기 RCF 기반 온도 패턴 이상 분석 작업."""
    if rcf_anomaly_service is None:
        return {
            "success": False,
            "skipped": True,
            "reason": "RCF_ANOMALY_SERVICE_NOT_AVAILABLE",
        }

    async def _run():
        try:
            from backend.database.connection import AsyncSessionLocal
            from backend.database.models import Factory
        except Exception:
            from database.connection import AsyncSessionLocal
        ...
        async with AsyncSessionLocal() as db:
            factory_result = await db.execute(select(Factory.factory_id))
            factory_ids = [row[0] for row in factory_result.all()]

            for factory_id in factory_ids:
                result = await rcf_anomaly_service.run_rcf_temperature_analysis(
                    db=db,
        ...
        return {
            "success": True,
            "checked_factory_count": len(results),
            "results": results,
        }

    result = asyncio.run(_run())
    print(f"[Job D] RCF 분석 완료: checked_factory_count={result.get('checked_factory_count')}")
    return result
...
def configure_scheduler_jobs() -> Any:
    """현재 단계에서는 Job A(30분 주기)와 Job C(1분 주기 이상 감지)와 Job D(5분 주기 rcf 기반 이상 감지)만 스케줄러에 등록한다."""
